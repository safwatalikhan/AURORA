...
def listHeuristics(dc,device,logFile,clickable_components):
    from DataPreparation import interactions
    import random
    actExplored = [dc.get_current_activity(device)]
    ccChanged=0
    
    dp.updateLog(logFile,"LIST","heuristics")
    
    listItems=[comp for comp in clickable_components if "textview" in comp.component_class.lower()]
    if listItems:
        interactions.pressButton(dc,device,clickable_components,random.choice(listItems),logFile)
